...
def responder_relato(aluno_id: str, relato_sk: str, texto: str, personal_id: str) -> bool:
    ...
    anotif_service.criar(aluno_id, tipo_notif, "Resposta do seu personal",
                         ...
                         ref_extra={"exercicio_id": updated.get("exercicio_id"),
                                    "chave": updated.get("chave"),
                                    "exercicio_nome": updated.get("exercicio_nome"),
                                    "relato_sk": relato_sk})
    return True
...
def adicionar_comentario(aluno_id: str, relato_sk: str, ator: str,
                         texto: str | None, midias: list[dict] | None = None) -> dict | None:
    """Appends a comment to a DOR/DUVIDA/POST thread. Retorna o item atualizado (os
    chamadores leem exercicio_id/chave/nome dele — nunca parsear a SK, que em posts novos
    carrega a chave no lugar do id) ou None se o item não existe."""

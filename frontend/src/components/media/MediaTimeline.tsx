...
export function MediaTimeline({ items, isLoading }: { items: MediaTimelineItem[]; isLoading?: boolean }) {
  ...
  return (
    <Card variant="elevated">
      ...
      <div className="space-y-3">
        {groups.map((g) => (
          <div key={g.data}>
            <p className="text-xs text-text-muted mb-1">{g.data}</p>
            <div className="flex gap-2 overflow-x-auto pb-1">
              {g.items.map((item) => (
                <button key={item.midia_id} onClick={() => setLightbox(item)} className="shrink-0 relative">
                  ...
                  <span
                    className="absolute -bottom-1 -right-1 p-0.5 rounded-full bg-surface-elevated border border-border"
                    title={item.ator === 'PERSONAL' ? 'Personal' : 'Aluno'}
                  >
                    {item.ator === 'PERSONAL' ? <UserCog size={10} /> : <User size={10} />}
                  </span>
                </button>
              ))}
            </div>

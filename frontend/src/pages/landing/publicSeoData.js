...
export const PAGES = {
  ...
  termos: {
    ...
    intro: 'Estes termos regulam o uso da plataforma CoachPilot (coachpilot.com.br) por personal trainers, studios e seus alunos. Ao criar uma conta, você concorda com as condições abaixo. Última atualização: 15 de agosto de 2026.',
    ...
    sections: [
      ...
      { title: '8. Conexão com assistentes de IA externos', body: 'O personal pode conectar sua conta a assistentes de IA de terceiros (como ChatGPT, Claude ou Gemini) pelo servidor MCP do CoachPilot. A conexão é opcional, exige autorização explícita no portal, alcança apenas os dados da própria conta e pode ser revogada a qualquer momento em Configurações → Conexões. Ao conectar, o personal autoriza o envio ao provedor escolhido dos dados que consultar pelo assistente — incluindo dados de saúde de seus alunos, como anamnese e avaliações físicas —, que passam a ser tratados também sob os termos daquele provedor. Como controlador dos dados dos alunos, cabe ao personal dispor de base legal para esse compartilhamento. O CoachPilot registra em auditoria as alterações feitas por essa via, notifica o personal a cada escrita e permite desfazer alterações de treino por 7 dias; a prescrição resultante permanece sob a responsabilidade técnica do personal.' },
      { title: '9. Disponibilidade e suporte', body: 'O CoachPilot emprega esforços comerciais razoáveis para manter o serviço disponível, hospedado em infraestrutura de nuvem de mercado. Manutenções programadas e eventos de força maior podem gerar indisponibilidades temporárias. O suporte é prestado em português, pelo WhatsApp +55 13 98808-8204.' },
      { title: '10. Alterações destes termos', body: 'Estes termos podem ser atualizados para refletir mudanças no produto ou na legislação. Alterações relevantes serão comunicadas pelo portal ou por e-mail. O uso contínuo da plataforma após a comunicação constitui aceite da nova versão.' },
      { title: '11. Legislação e foro', body: 'Estes termos são regidos pela legislação brasileira, incluindo o Código de Defesa do Consumidor e a Lei Geral de Proteção de Dados (Lei 13.709/2018). Fica eleito o foro do domicílio do usuário para dirimir controvérsias.' },
    ],
    faqs: [],
  ...
  privacidade: {
    ...
    intro: 'Esta política explica como o CoachPilot coleta, usa, armazena e protege dados pessoais, em conformidade com a Lei Geral de Proteção de Dados (LGPD — Lei 13.709/2018). Última atualização: 15 de agosto de 2026.',
    bullets: ['Dados usados apenas para prestar o serviço', 'Sem venda de dados a terceiros', 'Conexão com IA externa é opcional e revogável', 'Infraestrutura em nuvem com criptografia', 'Direitos LGPD atendidos via WhatsApp de suporte'],
    sections: [
      ...
      { title: '6. Conexão com assistentes de IA externos', body: 'O personal pode conectar sua conta a assistentes de IA de terceiros (ChatGPT/OpenAI, Claude/Anthropic, Gemini/Google) pelo servidor MCP do CoachPilot. A conexão é opcional, exige autorização explícita por OAuth no portal e dá acesso apenas aos dados da conta daquele personal — nunca aos de outras contas. Com a conexão ativa, os dados que o personal consultar pelo assistente são transmitidos ao provedor escolhido para gerar a resposta, o que inclui dados pessoais sensíveis de saúde dos alunos (anamnese, avaliações físicas e relatos de dor) quando o personal os solicita. Esses provedores estão sediados fora do Brasil, o que caracteriza transferência internacional de dados nos termos dos arts. 33 e seguintes da LGPD, e o tratamento passa a se reger também pela política daquele provedor. No momento da autorização o personal escolhe conceder apenas leitura ou também escrita de treinos, e pode revogar a conexão a qualquer momento em Configurações → Conexões, com efeito imediato. Não há envio por iniciativa do CoachPilot: a transmissão só ocorre quando o personal faz uma consulta no assistente com a conexão ativa. Toda escrita por essa via é registrada em auditoria e notificada ao personal.' },
      { title: '7. Cookies e analytics', body: 'O site usa armazenamento local para preferências (como tema claro/escuro) e sessão de login. Podemos utilizar ferramentas de métricas de audiência nas páginas públicas para entender o uso do site; essas métricas não são cruzadas com os dados de alunos da plataforma.' },
      { title: '8. Retenção e exclusão', body: 'Os dados são mantidos enquanto a conta estiver ativa. Ao excluir um aluno ou encerrar a conta, os dados correspondentes são removidos dos sistemas ativos em prazo razoável, ressalvadas obrigações legais de guarda (como registros fiscais de pagamento).' },
      { title: '9. Seus direitos', body: 'Titulares podem solicitar confirmação de tratamento, acesso, correção, portabilidade, anonimização ou exclusão de seus dados, nos termos da LGPD. Alunos devem direcionar solicitações primeiro ao seu personal (controlador); o CoachPilot dará suporte técnico ao atendimento. Canal para exercício de direitos: WhatsApp +55 13 98808-8204.' },
      { title: '10. Segurança', body: 'Adotamos controles de acesso por autenticação, criptografia em trânsito (HTTPS) e em repouso, isolamento de dados por conta e princípio de menor privilégio na infraestrutura. Nenhum sistema é imune a incidentes; em caso de incidente de segurança relevante, os afetados e as autoridades serão comunicados conforme a LGPD.' },
      { title: '11. Alterações desta política', body: 'Esta política pode ser atualizada para refletir mudanças no produto ou na legislação. A versão vigente estará sempre publicada em coachpilot.com.br/privacidade, com a data de atualização no topo.' },
    ],
    faqs: [],
